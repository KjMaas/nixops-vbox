...
class VirtualBoxState(MachineState[VirtualBoxDefinition]):
    ...
    def _get_vm_state(self, can_fail=False):
        ...
        if "VMState" not in vminfo:
            raise Exception(
                "unable to get state of VirtualBox VM ‘{0}’".format(self.name)
            )
    ...
    def create(  # noqa: C901
        ...
        self._logged_exec(
            [
                ...
                "set",
                self.vm_id,
                "/VirtualBox/GuestInfo/NixOps/PrivateHostEd25519Key",
                self.private_host_key,
            ]
        )
        ...
        if not os.path.isdir(vm_dir):
            raise Exception(
                "can't find directory of VirtualBox VM ‘{0}’".format(self.name)
            )
        ...
        for sf_name, sf_state in list(self.shared_folders.items()):
            if sf_name not in defn.config.virtualbox.sharedFolders:
                if not self.started:
                    ...
                    self.warn(
                        "skipping removal of shared folder ‘{0}’ since VirtualBox machine is running".format(
                            sf_name
                        )
                    )
        ...
        for disk_name, disk_state in list(self.disks.items()):
            if disk_name not in defn.config.virtualbox.disks:
                if not self.depl.logger.confirm(
                    ...
                    raise Exception(
                        "not destroying VirtualBox disk ‘{0}’".format(disk_name)
                    )
    ...
    def destroy(self, wipe=False):
        ...
        if not self.depl.logger.confirm(
            "are you sure you want to destroy VirtualBox VM ‘{0}’?".format(self.name)
        ):
            return False
        ...
        if vmstate == "running":
            self._logged_exec(
                ["VBoxManage", "controlvm", self.vm_id, "poweroff"], check=False
            )
